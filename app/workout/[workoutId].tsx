...
import { firestoreService, UserWorkout } from "../../services/firestoreService";
import {
  workoutGenerator,
...
export default function WorkoutSessionScreen() {
  ...
  const loadExerciseGif = async (exerciseId: string) => {
    try {
      const url = await exerciseApi.getExerciseGifUrl(exerciseId);
      setGifUrl(url);
    } catch (error) {
      console.warn("Failed to load exercise GIF:", error);
      if (workout) {
      ...
      const workoutData: UserWorkout = {
        ...
        exercises: workout.exercises.map((ex) => ({
          exercise: {
            ...
            bodyPart: ex.exercise.bodyPart || "general",
            target: ex.exercise.target || "general",
            equipment: ex.exercise.equipment || "body weight",
